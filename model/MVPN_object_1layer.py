# 1-layer MVPN - cope3(object) prediction
import sys, os, time
...
save_dir = '/gsfs0/data/fangmg/MVPD/model/model_runs/MVPD_net_' + sub + '/' + sub + '_cope3_object_1layer100_relu/MVPD_net_' + sub + '_testrun' + str(this_run) + '_epoch' + str(num_epochs) 
...
if __name__ == "__main__":
    # Training 
    object_train = object_dataset.Object_Dataset()
    object_train.get_train(this_run, total_run)
    trainloader = DataLoader(object_train, batch_size, shuffle=True, num_workers=0, pin_memory=True) 
    # Testing 
    object_test = object_dataset.Object_Dataset()
    object_test.get_test(this_run, total_run)
    testloader = DataLoader(object_test, batch_size, shuffle=False, num_workers=0, pin_memory=True) 
