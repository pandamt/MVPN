# 5-layer dense MVPN - cope 1-4 total prediction
import sys, os, time
...
save_dir = '/gsfs0/data/fangmg/MVPD/model/model_runs/MVPD_net_' + sub + '/' + sub + '_copeAll_5layer100_dense/MVPD_net_' + sub + '_testrun' + str(this_run) + '_epoch' + str(num_epochs) 
# create output folder if not exists
if not os.path.exists(save_dir):
       os.mkdir(save_dir)

# Hyper-parameters of NN 
input_size = 880 # cope 1-4, each 80 voxels
hidden_size = 100
...
if __name__ == "__main__":
    # Training 
    total_train = total_dataset.Total_Dataset()
    total_train.get_train(this_run, total_run)
    trainloader = DataLoader(total_train, batch_size, shuffle=True, num_workers=0, pin_memory=True) 
    # Testing 
    total_test = total_dataset.Total_Dataset()
    total_test.get_test(this_run, total_run)
    testloader = DataLoader(total_test, batch_size, shuffle=False, num_workers=0, pin_memory=True) 
