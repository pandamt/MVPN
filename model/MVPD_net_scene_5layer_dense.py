...
save_dir = '/gsfs0/data/fangmg/MVPD/model/model_runs/MVPD_net_' + sub + '/' + sub + '_cope4_scene_5layer100_dense/MVPD_net_' + sub + '_testrun' + str(this_run) + '_epoch' + str(num_epochs) 
# create output folder if not exists
if not os.path.exists(save_dir):
       os.mkdir(save_dir)

# Hyper-parameters of NN 
input_size = 240 # PPA, TOS, RSP each 80 voxels
hidden_size = 100
...
if __name__ == "__main__":
    # Training 
    scene_train = scene_dataset.Scene_Dataset()
    scene_train.get_train(this_run, total_run)
    trainloader = DataLoader(scene_train, batch_size, shuffle=True, num_workers=0, pin_memory=True) 
    # Testing 
    scene_test = scene_dataset.Scene_Dataset()
    scene_test.get_test(this_run, total_run)
    testloader = DataLoader(scene_test, batch_size, shuffle=False, num_workers=0, pin_memory=True) 
