...
class Face_Dataset(Dataset):
    ...
    def get_train(self, this_run=0, total_run=0):
        ...
        for run in it.chain(range(1,this_run), range(this_run+1,total_run+1)):
                ...
                STS_data = np.load(ROI_dir + str(run) + '_ROIs/' + sub + '_cope1/STS_80vox.npy')
                ...
                ROI_data_run = np.concatenate([FFA_data, STS_data], 1)
    ...
    def get_test(self, this_run=0, total_run=0): 
        ...
        STS_data = np.load(ROI_dir + str(this_run) + '_ROIs/' + sub + '_cope1/STS_80vox.npy')
        OFA_data = np.load(ROI_dir + str(this_run) + '_ROIs/' + sub + '_cope1/OFA_80vox.npy')
        ROI_data = np.concatenate([FFA_data, STS_data], 1)
